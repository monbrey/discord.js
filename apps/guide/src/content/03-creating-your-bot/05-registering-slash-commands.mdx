...
<Alert title="Read first!" type="info">
    ...
    1. The [individual command files](./adding-commands), containing their definitions and functionality.
    2. The [command handler](./handling-commands-interactions), which dynamically reads the files and executes the commands.
...
- utilising the different [Response methods](../slash-commands/command-response-methods) that can be used for slash commands.
- expanding on these examples with additional validated option types in [Advanced command creation](../slash-commands/advanced-command-creation).
